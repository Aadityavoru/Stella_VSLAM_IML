...
int stereo_tracking(const std::shared_ptr<stella_vslam::system>& slam,
    ...
    std::thread thread([&]() {
        while (is_not_end) {
            ...
            std::cout << "Before reading frame" << std::endl;
            is_not_end = video.read(frame);
            std::cout << "After reading frame" << std::endl;
            if (frame.empty()) {
                continue;
            }
            // Print the input frame size
            std::cout << "Input frame size: " << frame.cols << " x " << frame.rows << std::endl;

            // Ensure that the frame width is divisible by 2 before splitting
            if (frame.cols % 2 != 0) {
                spdlog::critical("Input image width is not divisible by 2. Cannot split into stereo frames.");
                return EXIT_FAILURE;
            }

            // Split the frame into left and right frames (assuming horizontal split)
            ...
            // Print the left and right frame sizes
            ...
            // cv::imshow("Left Frame", left_frame);   // Display left frame
            // cv::imshow("Right Frame", right_frame); // Display right frame
            // cv::waitKey(1); // Small delay to allow window refresh; adjust as needed (1 ms delay)
            ...
            // Rectify the frames
            rectifier.rectify(left_frame, right_frame, frames_rectified[0], frames_rectified[1]);
            ...
            std::chrono::system_clock::time_point now = std::chrono::system_clock::now();
            double timestamp = std::chrono::duration_cast<std::chrono::duration<double>>(now.time_since_epoch()).count();
            // Declare variables to hold the resized frames
            cv::Mat resized_left_frame, resized_right_frame;

            // Resize the frames to 640x320
            cv::resize(frames_rectified[0], resized_left_frame, cv::Size(640, 480));
            cv::resize(frames_rectified[1], resized_right_frame, cv::Size(640, 480));

            // Display the resized frames
            cv::imshow("Left frame", resized_left_frame);
            cv::imshow("Right frame", resized_right_frame);

            cv::waitKey(1);
            slam->feed_stereo_frame(frames_rectified[0], frames_rectified[1], timestamp, mask);
            slam->get_frame_publisher()->update_frames(frames_rectified[0], frames_rectified[1]);
            const auto tp_2 = std::chrono::steady_clock::now();
